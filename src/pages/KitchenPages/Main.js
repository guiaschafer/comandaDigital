...
class OrderHistory extends React.Component {
    ...
    getOrders = async () => {
        ...
        let orders = await axios.get("https://comandadigitalbackend.azurewebsites.net/ordersKitchen", {
            headers: headers
        }).then(response => {
            let ordersHistory = response.data;
    ...
    componentDidMount() {
        ...
        this.intervalo = setInterval(this.getOrders, 1000);
    }
    ...
    async updateStatusOrder(orderDetail) {
        ...
        if (orderDetail.codigoStatus == 1 || orderDetail.codigoStatus == 0) {
            novoStatus = 2
        } else {
            novoStatus = 4
        }
                    ...
                    <View style={styles.viewOrder}>
...

                        {
                            orders.map((order, index) => {
                                return <OrderCard orderDetail={order} updateStatusOrder={async (order) => this.updateStatusOrder(order)} navigation={this.props.navigation} key={`order-card-${index}`} />
                            })
                        }
                    </View>
                </ScrollView>
